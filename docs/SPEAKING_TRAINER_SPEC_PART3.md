...
> ⚠️ **ИСТОРИЧЕСКИЙ ДОКУМЕНТ (2026-09-06, bd i0r):** изначальная спека реализации (admin/grading). Канон текущего поведения — `openspec/specs/` (v1.x). Устаревшие формулировки помечены, НЕ удалялись.
...
~~**Повторные отправки ученика** на тот же топик — отдельные строки inbox, никакой дедупликации на фронте (PRD Edge Case).~~ **УСТАРЕЛО (2026-09-06, bd i0r/qbq.1):** одна отправка на (ученик, топик) — 409 DUPLICATE_SUBMISSION; фронт при 409 показывает состояние «уже отправлено». Канон: `openspec/specs/speaking-practice` v1.1.
