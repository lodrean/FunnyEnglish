...
class PublicAdaptiveService(
    ...
    private fun com.funnyenglish.entity.Question.toSharedQuestion(): Question {
            ...
            entity?.let {
                ...
                ImageWordMatchContent(
                    ...
                    hotspots = hotspots.map { h ->
                        com.funnyenglish.shared.model.HotspotData(
                            id = h.hotspotId,
                            x = h.x,
                            y = h.y,
                            width = h.width,
                            height = h.height,
                            shape = com.funnyenglish.shared.model.HotspotShape.RECTANGLE,
                            wordId = h.wordId
                        )
                    },
                    words = words.map { w ->
                        com.funnyenglish.shared.model.WordData(
                            id = w.wordId,
                            text = w.text,
        ...
        return Question(
            id = this.id.toString(),
            type = when (this.type) {
                QuestionType.TEXT_SELECT -> com.funnyenglish.shared.model.QuestionType.TEXT_SELECT
                QuestionType.AUDIO_SELECT -> com.funnyenglish.shared.model.QuestionType.AUDIO_SELECT
                QuestionType.IMAGE_SELECT -> com.funnyenglish.shared.model.QuestionType.IMAGE_SELECT
                QuestionType.FILL_BLANK -> com.funnyenglish.shared.model.QuestionType.FILL_BLANK
                QuestionType.DRAG_DROP_IMAGE -> com.funnyenglish.shared.model.QuestionType.DRAG_DROP_IMAGE
                QuestionType.IMAGE_WORD_MATCH -> com.funnyenglish.shared.model.QuestionType.IMAGE_WORD_MATCH
                else -> com.funnyenglish.shared.model.QuestionType.TEXT_SELECT
            },
            title = this.title,
            text = this.text,
    ...
    private fun com.funnyenglish.entity.Answer.toSharedAnswer(): Answer {
        return Answer(
            id = this.id.toString(),
            text = this.text,
            imageUrl = this.imageUrl,
