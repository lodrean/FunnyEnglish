...
export default defineConfig({
  ...
  expect: {
    timeout: 5000,
    // Допуск на антиалиасинг шрифтов: базлайны генерируются в mcr playwright-образе,
    // CI — на GH-раннере (`playwright install --with-deps`) — freetype/fontconfig дают
    // суб-пиксельные различия текста (~1% пикселей, bd 9wf). Реальные регрессии
    // (сдвиги, пропавшие блоки) дают на порядки больше.
