# Структура проекта FunnyEnglish (монорепо)
...
- composeApp/ — ОСНОВНОЙ KMP UI-модуль (android/ios/desktop/wasmJs). Монолит: app/screens/*, app/viewmodel/*, app/di/*. Точка входа desktop: com.funnyenglish.app.MainKt; App.kt — KoinApplication + ручная навигация (sealed AppScreen + mutableStateOf, БЕЗ NavHost).
- app/ — тонкая Android-обёртка (FunnyEnglishApplication, MainActivity).
- shared/ — KMP: api/FunnyEnglishApi.kt (Ktor), model/, platform/, GuestProgressRepository.
